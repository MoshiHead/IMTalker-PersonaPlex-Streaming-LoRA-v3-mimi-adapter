...
class LiveHeliumFMEngine:
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.last_motion_frame: torch.Tensor | None = None
    ...
    def _sample_motion_from_helium(self, helium: torch.Tensor, target_frames: int) -> tuple[torch.Tensor, dict]:
        ...
        self.last_motion_frame = motion[-1:].detach().clone()
        return motion, timings

    @torch.no_grad()
    def _silent_motion(self, target_frames: int) -> tuple[torch.Tensor, dict]:
        """Freeze the avatar on the last rendered frame instead of running FM.

        Used during confirmed silence (no Moshi reply audio) so the avatar
        does not produce random lip movement from FM-sampled noise.
        """
        target_frames = max(1, int(target_frames))
        if self.last_motion_frame is not None:
            motion = self.last_motion_frame.expand(target_frames, -1).contiguous()
        else:
            motion = torch.zeros(target_frames, 32, device=self.device, dtype=torch.float32)
        timings: dict = {
            "helium_ms": 0.0,
            "fm_ms": 0.0,
                    ...
                    for ev in reply_engine.process_ready_steps_limited(max_moshi_steps_per_loop):
                        ...
                        if bool(getattr(args, "direct_reply_hidden", False)):
                            ...
                            if is_speech:
                                ...
                                # Avatar is idle: freeze on the last frame instead of
                                # running FM on near-silent Helium hidden states, which
                                # otherwise produces random low-amplitude lip movement.
                                motion, fm_info = fm_engine._silent_motion(target_frames)
                            used_codes = [
