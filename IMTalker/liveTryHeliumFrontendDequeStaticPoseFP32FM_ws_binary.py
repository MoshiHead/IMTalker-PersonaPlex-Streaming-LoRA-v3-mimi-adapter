...
# Studio frontend adapter input sources (--frontend_source).
# "helium": Moshi LM transformer hidden state, layer[-2], dim 4096 @ 12.5Hz.
#   Carries fused text/dialogue context (see lm.py text_emb fusion) in addition
#   to acoustic content, which is the suspected source of random lip movement
#   during silence (the adapter reads text-driven drift as motion signal).
# "mimi_decoder": Mimi decoder_transformer latent, dim 512 @ 12.5Hz (post-quantizer,
#   pre-SEANet-decoder). Purely acoustic, zero text context.
HELIUM_HIDDEN_DIM = 4096
...
class StudioNativeLiveAdapter(nn.Module):
    ...
    def __init__(
        ...
        num_layers: int,
        dropout: float = 0.0,
        in_dim: int = HELIUM_HIDDEN_DIM,
    ) -> None:
        super().__init__()
        self.model = HeliumToWav2VecFrontendAdapter(
            helium_dim=int(in_dim), num_layers=int(num_layers), dropout=float(dropout)
        )
...
class MoshiOnlyEngineWithHidden(MoshiOnlyEngine):
    ...
    def __init__(
        ...
        *args,
        capture_layer: int = -2,
        capture_mimi_decoder_hidden: bool = False,
        **kwargs,
    ) -> None:
        self.tf_capture_layer = int(capture_layer)
        self.capture_mimi_decoder_hidden = bool(capture_mimi_decoder_hidden)
        super().__init__(*args, **kwargs)
        self._install_graph_hidden_capture()
        if self.capture_mimi_decoder_hidden:
            self._install_mimi_decoder_hidden_capture()

    def _install_mimi_decoder_hidden_capture(self) -> None:
        """Monkeypatch Mimi.decode to also expose the decoder_transformer's latent.

        Mimi runs with mimi.streaming_forever(1), so decode() always takes the
        `state.graphed_tr_dec(emb)` branch (see compression.py MimiModel.decode).
        CUDAGraphed.__call__ replays a captured CUDA graph on every call after
        the first and does NOT re-invoke the wrapped module's Python forward,
        so a plain register_forward_hook on decoder_transformer silently stops
        firing once the graph is captured (moshi/utils/compile.py CUDAGraphed).
        We instead inline decode()'s body so the same call that already runs
        the decoder_transformer also stashes its output before the final
        (and here-unneeded) SEANet decoder conv stack.
        """
        mimi = self.mimi
        mimi._last_decoder_hidden = None

        def decode_with_hidden(self_mimi, codes: torch.Tensor):
            state = self_mimi._streaming_state
            emb = self_mimi.decode_latent(codes)
            emb = self_mimi._to_encoder_framerate(emb)
            if self_mimi.decoder_transformer is not None:
                if state is None:
                    (emb,) = self_mimi.decoder_transformer(emb)
                else:
                    (emb,) = state.graphed_tr_dec(emb)
            self_mimi._last_decoder_hidden = emb.detach()
            with self_mimi._context_for_encoder_decoder:
                out = self_mimi.decoder(emb)
            return out

        mimi.decode = types.MethodType(decode_with_hidden, mimi)
        print("[liveTryPlasticity] installed Mimi decoder_transformer hidden capture", flush=True)
    ...
    def _step(self, pcm24: np.ndarray) -> dict:
        ...
        if tokens is None:
            ...
            if self.capture_mimi_decoder_hidden:
                raw = getattr(self.mimi, "_last_decoder_hidden", None)
                if isinstance(raw, torch.Tensor):
                    # raw: [B, 512, T] conv-layout latent @ encoder_frame_rate (25Hz).
                    # One LM step covers 80ms (12.5Hz) -> T=2 frames here. Keep only
                    # the most recent frame so this lines up 1:1 with one 12.5Hz step,
                    # mirroring helium_hidden's `layer_hidden[:1, -1:]` semantics, and
                    # downstream deque/interpolation code needs no further changes.
                    mimi_hidden = raw[:1, :, -1:].transpose(1, 2).detach().float().cpu()
...
def _load_fm(args: argparse.Namespace, device: torch.device) -> FMGenerator:
    ...
    lora_path = getattr(args, "lora_path", "") or ""
    if lora_path and not os.path.exists(lora_path):
        print(f"[liveTryHeliumFM][LoRA] lora_path={lora_path} not found, skipping LoRA", flush=True)
        lora_path = ""

    if lora_path:
        include_cam = not args.no_lora_cam_projection
        if getattr(args, "cam_condition_mode", "normal") in ("remove", "zero_embedding"):
            include_cam = False
        apply_lora_to_model(
            fm,
            rank=args.lora_rank,
            ...
            dropout=0.0,
            include_pose_lora=not args.no_lora_pose_projection,
            include_audio_lora=not args.no_lora_audio_projection,
            include_cam_lora=include_cam,
            only_pose_lora=args.only_lora_pose_projection,
        )
        lora_ckpt = torch.load(lora_path, map_location="cpu")
        lora_raw = lora_ckpt.get("ema_state_dict") or lora_ckpt.get("state_dict", lora_ckpt.get("model", lora_ckpt))
        if isinstance(lora_raw, dict) and "model" in lora_raw and isinstance(lora_raw["model"], dict):
            lora_raw = lora_raw["model"]
        lora_cleaned = {
            k.replace("model.", "", 1) if k.startswith("model.") else k: v for k, v in lora_raw.items()
        }
        lmissing, lunexpected = fm.load_state_dict(lora_cleaned, strict=False)
        lora_keys = sum(1 for k in lora_cleaned if "lora_" in k)
        fm = fm.to(device).eval()
        print(
            f"[liveTryHeliumFM][LoRA] loaded={lora_path} lora_keys={lora_keys} "
            f"missing={len(lmissing)} unexpected={len(lunexpected)}",
            flush=True,
        )
...
class LiveHeliumFMEngine:
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        # Frontend fp32 adapter: raw 12.5Hz Helium (or Mimi decoder latent) ->
        # Wav2Vec2 frontend -> frozen Wav2Vec2 final hidden.
        self.frontend_source = str(getattr(args, "frontend_source", "helium"))
        if self.frontend_source not in ("helium", "mimi_decoder"):
            raise ValueError(f"Unknown --frontend_source: {self.frontend_source!r}")
        if self.frontend_source == "mimi_decoder":
            adapter_in_dim = MIMI_DECODER_HIDDEN_DIM
            adapter_ckpt_path = getattr(args, "mimi_adapter_path", "")
            if not adapter_ckpt_path:
                raise ValueError("--mimi_adapter_path is required when --frontend_source=mimi_decoder")
        else:
            adapter_in_dim = HELIUM_HIDDEN_DIM
        ...
        payload = torch.load(adapter_ckpt_path, map_location="cpu")
        ...
        print(
            ...
            f"source={self.frontend_source} path={adapter_ckpt_path} layers={args.adapter_num_layers}",
            flush=True,
        )
    ...
    def _warmup(self) -> None:
        ...
        if self.extractor is None:
            ...
            dummy_dim = MIMI_DECODER_HIDDEN_DIM if self.frontend_source == "mimi_decoder" else HELIUM_HIDDEN_DIM
            dummy_helium = torch.zeros(raw_steps, dummy_dim, device=self.device, dtype=torch.float32)
...
class LiveHeliumFMOptions(BaseOptions):
    def initialize(self, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        parser.add_argument("--lora_path", default="", help="Optional LoRA adapter checkpoint applied on top of generator_path")
        ...
        parser.add_argument(
            "--frontend_source",
            choices=["helium", "mimi_decoder"],
            default="helium",
            help=(
                "Source tap for the studio frontend adapter. 'helium' (default, unchanged "
                "behavior): Moshi LM transformer hidden, carries fused text/dialogue context. "
                "'mimi_decoder': Mimi decoder_transformer latent, acoustic-only, no text "
                "context -- requires --mimi_adapter_path."
            ),
        )
        parser.add_argument(
            "--mimi_adapter_path",
            default="",
            help="Frontend fp32 Mimi-decoder->Wav2Vec2 projected-frontend adapter checkpoint (required when --frontend_source=mimi_decoder)",
        )
    ...
    def get_moshi_engine() -> MoshiOnlyEngine:
        ...
        if moshi_engine is None:
            moshi_engine = MoshiOnlyEngineWithHidden(
                ...
                capture_mimi_decoder_hidden=(getattr(args, "frontend_source", "helium") == "mimi_decoder"),
            )
        return moshi_engine
                ...
                while not stopped:
                    ...
                    for ev in reply_engine.process_ready_steps_limited(max_moshi_steps_per_loop):
                        ...
                        fm_engine._session_reply_events.append({
                            ...
                            "hidden": bool(
                                isinstance(ev.get("helium_hidden"), torch.Tensor)
                                or isinstance(ev.get("mimi_hidden"), torch.Tensor)
                            ),
                        ...
                        if bool(getattr(args, "direct_reply_hidden", False)):
                            hidden_key = (
                                "mimi_hidden"
                                if getattr(args, "frontend_source", "helium") == "mimi_decoder"
                                else "helium_hidden"
                            )
                            hidden = ev.get(hidden_key)
